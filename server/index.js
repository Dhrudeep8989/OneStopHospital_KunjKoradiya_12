...
// Patient API
app.get('/api/vaccine_reminders', (req, res) => {
  ...
  const sql = `
    SELECT vaccine_name, date_of_vaccine 
    FROM vaccines 
    WHERE p_id = ? AND date_of_vaccine >= CURDATE()
  `;
  db.query(sql, [patientId], (err, results) => {
    if (err) {
      console.error('Error fetching vaccine reminders:', err);
      return res.status(500).json({ error: 'Database error' });
    }
    res.json(results);
  });
...
app.get('/api/medicine_reminders', (req, res) => {
  const patientId = req.query.patient_id;
  if (!patientId) {
    return res.status(400).json({ error: 'Patient ID is required' });
  }
  ...
  const sql = `
    SELECT b.medicine_id, m.medicinename, b.from_medicine, b.to_medicine, b.M, b.A, b.N 
    FROM bills b
    JOIN medicines m ON b.medicine_id = m.id
    WHERE b.p_id = ? AND b.from_medicine <= CURDATE() AND b.to_medicine >= CURDATE()
  `;
  db.query(sql, [patientId], (err, results) => {
    if (err) {
...
// Add this endpoint to fetch date and disease from cases table
app.get('/get-case', (req, res) => {
  const sql = 'SELECT date, disease FROM cases';
...
// Add this endpoint to fetch reports based on patient_id
app.get('/api/reports', (req, res) => {
  ...
  const sql = `
    SELECT r.created_at, r.description, r.report_file
    FROM reports r
...
app.get('/api/downloadReport/:filename', (req, res) => {
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
      // Temporarily set a placeholder filename
...
app.post('/CreateReport', upload.single('File'), [
    ...
    db.query(caseQuery, [caseID], (err, results) => {
        ...
        const sql = 'INSERT INTO reports (case_id, doc_id, description, report_file, created_at, type) VALUES (?, ?, ?, ?, NOW(), ?)';
        ...
        db.query(sql, values, (err, result) => {
            ...
            const fs = require('fs');
            const oldPath = `./reports/${reportFile}`;
            const newPath = `./reports/${newFilename}`;
            ...
            fs.rename(oldPath, newPath, (err) => {
                ...
                // Update the report record with the new filename
                const updateSql = 'UPDATE reports SET report_file = ? WHERE id = ?';
                db.query(updateSql, [newFilename, reportID], (err) => {
                    if (err) {
                        console.error('Error updating report filename:', err);
                        return res.status(500).json({ message: 'Error updating report filename.' });
                    }

                    res.status(201).json({
                        id: reportID,
                        message: 'Report created successfully',
                        reportFile: newFilename
                    });
                });
            });
        });
